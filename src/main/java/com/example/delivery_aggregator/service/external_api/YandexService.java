...
import com.example.delivery_aggregator.dto.external_api.yandex.PricingSourceNode;
import com.example.delivery_aggregator.dto.external_api.yandex.YandexCalculatorRequest;
import com.example.delivery_aggregator.dto.external_api.yandex.YandexCalculatorResponse;
...
public class YandexService {
    ...
    @Value("${authorization-codes.yandex}")
    ...
    public HttpHeaders getHttpHeaders(){
        ...
        headers.setBearerAuth(yandexAuthToken);
        headers.set("Accept-Language","ru");
        return headers;
    }

    public ResponseEntity<YandexCalculatorResponse> getTariffs(IndexPageDataDto indexPageDataDto) {
        ...
        yandexCalculatorRequest.setPlaces(indexPageDataDto.getPackages().stream().map(yandexMapper:: packageToPricingResourcePlace).toList());
        ...
        int total_weight = 0;
        for (PackageDto p : indexPageDataDto.getPackages()) {
