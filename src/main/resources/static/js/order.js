 ...
 * Загружает данные из куки, заполняет форму, проверяет корректность и отправляет заказ
 */
import { checkAuthStatus, createOrder } from './common/api.js';
...
class OrderManager {
  ...
  isFormValid() {
    ...
    fields.forEach(({ id, validator }) => {
      ...
      if (!value || !validator(value)) {
        showValidateFormError(input, `${id}-error`, 'Пожалуйста, заполните поле корректно');
        isValid = false;
      }
    });
    return isValid;
  ...
  async sendOrder(event) {
    ...
    try {
      ...
      showErrorMessage(error.message || 'Произошла ошибка при отправке заказа');
    } finally {
      showSpinner(false);
    }
  }
